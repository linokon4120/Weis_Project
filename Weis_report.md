...
Completion rates are nearly identical across assignment types — Required (78.20%), Recommended (78.10%), Elective (78.10%), and Manager Assigned (79.00%) — a spread of less than 1 percentage point. Chi-square testing confirms this spread is statistically significant at n=100,000, but the 95% Wilson confidence intervals overlap substantially across all four groups. This is a case where statistical significance reflects dataset power rather than a meaningful operational difference; the assignment category a training record carries has virtually no bearing on whether it gets completed.

In contrast, the completion rate gap across departments is both statistically significant and practically large. A chi-square test across all departments is highly significant (p < 0.001), and a pairwise z-test confirms the top-performing department (Corporate Leadership, ~91.7%) versus the lowest (Regional Leadership, ~72.0%) gap is supported by non-overlapping 95% confidence intervals. This ~20-point spread is a genuine behavioral difference — the widest and most statistically robust segmentation signal in the LMS dataset — and likely reflects stronger accountability structures at the corporate level. It is the most actionable within-organization finding from the EDA.

##### Time Investment & Engagement

32.20% of all modules are past due — a significant operational concern. This overall past-due rate is also highly differentiated across stores, ranging from 13.8% in the most disciplined stores to 51.2% in the weakest, with a standard deviation of 6.8 percentage points. At the module category level, chi-square testing confirms that past-due rates differ significantly across categories, but the spread is narrow (roughly 2 percentage points from highest to lowest category). Crucially, pairwise comparisons between extreme categories should account for unequal sample sizes: Analytics (n≈987) has confidence intervals three to four times wider than high-volume categories like Operations or Compliance (n>20,000), meaning the most-cited category contrast may not be individually statistically significant even when the overall chi-square is. The actionable signal is the wide store-level spread (13.8%–51.2%), not the narrow category-level variation. Because past-due rate is strongly negatively correlated with completion rate (r = -0.92), these two metrics are effectively two sides of the same coin: stores that complete training on time have low past-due rates, and vice versa. This means both variables should not be used simultaneously in a single model due to multicollinearity — each one individually captures the full signal.
